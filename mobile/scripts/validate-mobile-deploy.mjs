...
const output=resolve(root,"dist-mobile"),files=(await readdir(output)).sort(),html=await readFile(resolve(output,"index.html"),"utf8"),config=await readFile(resolve(output,"mobile-config.js"),"utf8"),vercel=JSON.parse(await readFile(resolve(root,"vercel.json"),"utf8"));
const checks=[],check=(name,run)=>{run();checks.push(name)};
check("mobile output contains only deployable root assets",()=>assert.deepEqual(files,["favicon.ico","index.html","mobile-app.js","mobile-config.js","mobile-google-drive.js","mobile.css","shared"]));
