...
const root=new URL("../",import.meta.url),html=await readFile(new URL("web/mobile/index.html",root),"utf8"),syncSource=await readFile(new URL("web/shared/sync-state-model.js",root),"utf8"),repositorySource=await readFile(new URL("web/shared/project-repository.js",root),"utf8"),appSource=await readFile(new URL("web/mobile/mobile-app.js",root),"utf8"),css=await readFile(new URL("web/mobile/mobile.css",root),"utf8"),dom=new JSDOM(html,{url:"http://localhost/mobile/",runScripts:"outside-only"}),{window}=dom;
