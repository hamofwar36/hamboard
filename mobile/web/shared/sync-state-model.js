(function(root){
  ...
  const CLIENT_PROFILES=Object.freeze({
    desktop:Object.freeze({id:"desktop",collections:STATE_COLLECTIONS,userLibrary:true,workspace:true,workTracking:true}),
    mobileCore:Object.freeze({id:"mobile-core",collections:Object.freeze(STATE_COLLECTIONS.filter(([type])=>mobileTypes.has(type))),userLibrary:true,workspace:false,workTracking:false})
  });
  ...
  function applyWorkspace(target,workspace){
    if(!workspace||typeof workspace!=="object"||Array.isArray(workspace))return target;
    for(const field of workspaceFields)if(Object.prototype.hasOwnProperty.call(workspace,field))target[field]=clone(workspace[field]);
    if(workspace.settings&&typeof workspace.settings==="object"&&!Array.isArray(workspace.settings))target.settings={...(target.settings&&typeof target.settings==="object"?target.settings:{}),...clone(workspace.settings)};
    return target
  }
  ...
  function applyChangesToCanonical(canonicalState={},changes=[],{profile=CLIENT_PROFILES.desktop,strictProfile=false}={}){
    ...
    for(const change of Array.isArray(changes)?changes:[]){
      ...
      if(type==="user-library"){
        ...
        next.tagLibrary=clone(Array.isArray(change.payload.tagLibrary)?change.payload.tagLibrary:[]);next.favorites=clone(Array.isArray(change.payload.favorites)?change.payload.favorites:[]);if(profile.workspace)applyWorkspace(next,change.payload.workspace);continue
      }
  ...
  function projectChangesForClient(changes,profile=CLIENT_PROFILES.mobileCore){
    ...
    for(const change of Array.isArray(changes)?changes:[]){
      ...
      if(type==="user-library"){
        ...
        if(profile.workspace&&change.payload?.workspace&&typeof change.payload.workspace==="object"&&!Array.isArray(change.payload.workspace))payload.workspace=clone(change.payload.workspace);
  ...
  function diffClientProjection(baseProjection={},editedProjection={},profile=CLIENT_PROFILES.mobileCore){
    ...
    if(profile.userLibrary){const before={tagLibrary:Array.isArray(baseProjection?.tagLibrary)?baseProjection.tagLibrary:[],favorites:Array.isArray(baseProjection?.favorites)?baseProjection.favorites:[]},after={tagLibrary:Array.isArray(editedProjection?.tagLibrary)?editedProjection.tagLibrary:[],favorites:Array.isArray(editedProjection?.favorites)?editedProjection.favorites:[]};if(!same(before,after))changes.push({entityType:"user-library",entityId:"main",operation:"upsert",payload:clone(after)})}
